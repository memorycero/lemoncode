...
const averageMark = (califications: number[]): string => {
    return (califications
        .reduce((accumulator: number, calification: number): number => accumulator + calification)
        / califications.length).toPrecision(3);
}

const maxMark = (califications: number[]): number => {
...
const summarizeClassRoom = (studentList: Student[]): StudentSummary[] => {
    ...
    studentList.forEach(student =>
        result.push({
            name: student.name,
            highestCalification: maxMark(student.califications),
            averageCalifications: averageMark(student.califications)
        })
    );
    return result;
};
